    ...
    const carregarPedidosDoDia = async () => {
      try {
        ...
        const pedidosEntreguesHoje = Object.values(response.data).flatMap((motoboy) => {
          const pedidosFiltrados = motoboy.pedidos.filter(
            (pedido) => {
              const isEntregue = pedido.status === "Entregue";
              const dataFinalizacao = new Date(pedido.data_hora_finalizacao).toISOString().slice(0, 10);
              return isEntregue && dataFinalizacao === dataAtual;
            }
          );
        ...
        const groupedPedidos = pedidosEntreguesHoje.reduce((acc, pedido) => {
          ...
          if (!acc[nomeMotoboy]) {
            acc[nomeMotoboy] = { nome: nomeMotoboy, pedidos: [] };
          }
